...
import {GET_USER_ORGANIZED_CLEANDAYS} from '@api/user/endpoints.ts';
...
export function useGetUserOrganizedCleandays(userId: string | number, params: GetCleandayParams) {
    return useGetPaginatedManyTemplate<CleandayApiModel, Cleanday, GetCleandayResponse>(
        ['user', userId, 'organized'],
        substituteIdToEndpoint(userId, GET_USER_ORGANIZED_CLEANDAYS),
        'cleandays',
        params,
        {staleTime: 5 * 60 * 1000},
        cleandayMapper
